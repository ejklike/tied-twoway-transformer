...
class Scorer(object):
    def __init__(self, best_score_x2y, best_score_y2x, name):
        self.best_score_x2y = best_score_x2y
        self.best_score_y2x = best_score_y2x
        self.name = name
    ...
    def update(self, stats):
        self.best_score_x2y, self.best_score_y2x = self._caller(stats)
...
class PPLScorer(Scorer):
    ...
    def __init__(self):
        super(PPLScorer, self).__init__(float("inf"), float("inf"), "ppl")

    def is_improving(self, stats):
        return (stats.ppl('x2y') <= self.best_score_x2y and 
                stats.ppl('y2x') <= self.best_score_y2x)
    ...
    def _caller(self, stats):
        return stats.ppl('x2y'), stats.ppl('y2x')


class AccuracyScorer(Scorer):

    def __init__(self):
        super(AccuracyScorer, self).__init__(float("-inf"), float("-inf"), "acc")

    def is_improving(self, stats):
        return (stats.accuracy('x2y') >= self.best_score_x2y and
                stats.accuracy('y2x') >= self.best_score_y2x)
    ...
    def _caller(self, stats):
        return stats.accuracy('x2y'), stats.accuracy('y2x')
...
class EarlyStopping(object):
    ...
    def _update_increasing(self, valid_stats, step):
        ...
        for scorer in self.early_stopping_scorers:
            logger.info(
                "Model is improving {}: {:g} & {:g} --> {:g} & {:g}.".format(
                    scorer.name, scorer.best_score_x2y, scorer.best_score_y2x, *scorer(valid_stats))
            )
